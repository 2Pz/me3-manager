...
class ConfigManager:
    ...
    def __init__(self):
        ...
        if dynamic_profile_dir:
            ...
            if sys.platform == "win32":
                ...
                self.config_root = (
                    Path.home() / ".config" / "garyttierney" / "me3" / "profiles"
                )
        ...
        self.stored_custom_profile_paths = settings.get(
            "stored_custom_profile_paths", {}
        )
        ...
        if (
            current_settings.get("games") != self.games
            or current_settings.get("game_order") != self.game_order
        ):
            self._save_settings()
    ...
    def _migrate_external_mods_to_profile_system(self):
        ...
        for game_name, mods_list in current_tracked_mods.items():
            if isinstance(mods_list, list):
                new_tracked_mods[game_name] = {"default": mods_list}
    ...
    def _ensure_default_profiles(self):
        ...
        for game_name in self.games.keys():
            ...
            if not any(p["id"] == "default" for p in self.profiles[game_name]):
                self.profiles[game_name].insert(
                    0,
                    {
                        "id": "default",
                        "name": "Default",
    ...
    def validate_and_prune_profiles(self):
        ...
        for game_name, profile_list in list(self.profiles.items()):
            ...
            for p in profile_list:
                ...
                profile_file_ok = (
                    p.get("profile_path") and Path(p.get("profile_path")).is_file()
                )
            ...
            if len(valid_profiles) < original_count:
                removed_ids = set(p["id"] for p in profile_list) - set(
                    p["id"] for p in valid_profiles
                )
                ...
                if active_profile_id in removed_ids:
                    self.active_profiles[game_name] = "default"
                    print(
                        f"Active profile for {game_name} was pruned. Resetting to 'default'."
                    )
    ...
    def track_external_mod(self, game_name: str, mod_path: str):
        ...
        if game_name not in self.tracked_external_mods or not isinstance(
    ...
    def untrack_external_mod(self, game_name: str, mod_path: str):
        ...
        if game_name in self.tracked_external_mods and isinstance(
            ...
            profile_specific_mods = self.tracked_external_mods[game_name].get(
                active_profile_id
            )
    ...
    def check_and_reformat_profile(self, profile_path: Path):
        ...
        try:
            ...
            if (
                "[[natives]]" in content
                or "[[packages]]" in content
                or "[[supports]]" in content
            ):
                print(
                    f"Old profile format detected in {profile_path.name}. Reformatting."
                )
    ...
    def _normalize_and_deduplicate_config_paths(
            ...
            for native in config_data["natives"]:
                if isinstance(native, dict) and "path" in native:
                    ...
                    if normalized_path in seen_paths:
                        ...
                        current_options = len(
                            [k for k in native.keys() if k not in ["path", "enabled"]]
                        )
                        existing_options = len(
                            [
                                k
                                for k in existing_entry.keys()
        ...
        try:
            ...
            if tomllib:
                with open(config_path, "rb") as f:
                    ...
                    config_data = self._normalize_and_deduplicate_config_paths(
                        config_data
                    )
                    return config_data
    ...
    def _parse_config_fallback(self, config_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            native_matches = re.findall(
                native_pattern, content, re.MULTILINE | re.IGNORECASE
            )
            ...
            print(
                f"Found {len(native_matches)} native paths in profile: {native_matches}"
            )

            # Extract supports
            supports_pattern = r'\[\[supports\]\]\s*\n\s*(?:name\s*=\s*[\'"]([^\'"]+)[\'"]|game\s*=\s*[\'"]([^\'"]+)[\'"]|id\s*=\s*[\'"]([^\'"]+)[\'"])'
            supports_matches = re.findall(
                supports_pattern, content, re.MULTILINE | re.IGNORECASE
            )
            ...
            packages_matches = re.findall(
                packages_pattern, content, re.MULTILINE | re.DOTALL
            )
            ...
            for package in packages_to_write:
                ...
                if source_path:
                    if is_custom_profile:
                        ...
                        if Path(source_path).is_absolute() and source_path.startswith(
    ...
    def setup_file_watcher(self):
        ...
        if set(current_dirs) != target_dirs:
            ...
            if target_dirs_list:
                self.file_watcher.addPaths(target_dirs_list)
        ...
        if set(current_files) != target_files:
            ...
            if target_files_list:
                self.file_watcher.addPaths(target_files_list)
    ...
    def set_mod_advanced_options(
        self, game_name: str, mod_path: str, options: Dict[str, Any]
    ):
    ...
    def _update_native_advanced_options(
        self, config_data: Dict[str, Any], config_path: str, options: Dict[str, Any]
    ):
    ...
    def _update_package_advanced_options(
        self, config_data: Dict[str, Any], mod_name: str, options: Dict[str, Any]
    ):
        ...
        if package_entry is None:
            ...
            if options.get("enabled", False):
                ...
                mods_dir = (
                    self.get_mods_dir(self.game_name)
    ...
    def has_active_advanced_options(self, game_name: str, mod_path: str) -> bool:
        ...
        return any(
            key in advanced_options and advanced_options[key] for key in advanced_keys
        )

    def get_available_mod_names(
        self, game_name: str, mod_type: str = "all"
    ) -> List[str]:
    ...
    def write_me3_config(self, config_path: Path, native_paths: List[str]):
        ...
        if not config_data.get("supports"):
            game_name = (
                "nightreign" if "nightreign" in config_path.name else "eldenring"
            )
            config_data["supports"] = [{"game": game_name}]

        # Ensure packages section exists with default package
        if not config_data.get("packages"):
            mods_dir = self.games.get(
                "Nightreign" if "nightreign" in config_path.name else "Elden Ring", {}
            ).get("mods_dir", "")
            if mods_dir:
    ...
    def get_mods_info(self, game_name: str, skip_sync: bool = False) -> Dict[str, Dict]:
        ...
        if mods_dir.exists():  # Add a check to prevent errors if the dir was deleted
            for dll_file in mods_dir.glob("*.dll"):
                ...
                default_mods_dir_path = (
                    self.config_root / self.games[game_name]["mods_dir"]
                )
                ...
                if active_mods_dir == default_mods_dir_path:
                    ...
                    config_path = (
                        f"{self.games[game_name]['mods_dir']}\\{dll_file.name}"
                    )
                else:
            ...
            for folder in mods_dir.iterdir():
                if folder.is_dir() and folder.name != self.games[game_name]["mods_dir"]:
                    ...
                    if folder.name in acceptable_folders:
                        ...
                        if any(
                            sub.is_dir() and sub.name in acceptable_folders
        ...
        enabled_external_paths = {
            p.replace("\\", "/")
            for p in enabled_dll_paths
            if not Path(p).is_relative_to(self.config_root)
            and not p.startswith(self.games[game_name]["mods_dir"])
        }
    ...
    def remove_folder_mod(self, game_name: str, mod_name: str):
        ...
        if "packages" in config_data:
            config_data["packages"] = [
                pkg for pkg in config_data["packages"] if pkg.get("id", "") != mod_name
            ]
    ...
    def validate_and_fix_profile(self, profile_path: Path):
        ...
        try:
                ...
                for support in config_data["supports"]:
                    if isinstance(support, dict):
                        if "name" in support or "id" in support:
                            ...
                            if (
                                game_name.upper() == "ELDEN RING"
                                or game_name.upper() == "ER"
                            ):
                                game_name = "eldenring"
                            elif (
                                game_name.upper() == "NIGHTREIGN"
                                or game_name.upper() == "NR"
                            ):
                                game_name = "nightreign"
            ...
            if "packages" not in config_data or not config_data["packages"]:
                game_name = (
                    "nightreign" if "nightreign" in profile_path.name else "eldenring"
                )
                game_key = (
                    "Nightreign" if "nightreign" in profile_path.name else "Elden Ring"
                )
                mods_dir = self.games.get(game_key, {}).get("mods_dir", "")
    ...
    def sync_profile_with_filesystem(self, game_name: str):
        ...
        config_data = self._parse_toml_config(
            profile_path
        )  # This now includes normalization
        ...
        if (
            isinstance(game_profiles_mods, dict)
            and active_profile_id in game_profiles_mods
        ):
            original_tracked = game_profiles_mods[active_profile_id]
            # Normalize all paths before checking for existence
            valid_tracked = [
                p for p in original_tracked if Path(p.replace("\\", "/")).exists()
            ]
        ...
        if config_modified:
            print(
                f"Saving updated profile for '{game_name}' after syncing with filesystem and normalizing paths."
            )
            self._write_toml_config(profile_path, config_data)
    ...
    def simple_import_from_folder(
        self,
        game_name: str,
        import_folder: str,
        profile_file: str,
        merge: bool = True,
        custom_mod_name: str = None,
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            processed_dll_paths = {
                p.get("path", "").replace("\\", "/")
                for p in current_config.get("natives", [])
            }
            dll_paths_to_add = []
            ...
            for native in source_config.get("natives", []):
                ...
                full_path = (
                    import_path / original_path_str
                    if not Path(original_path_str).is_absolute()
                    else Path(original_path_str)
                )
                normalized_full_path = str(full_path).replace("\\", "/")
                normalized_original_path = original_path_str.replace("\\", "/")

                if (
                    normalized_full_path in processed_dll_paths
                    or normalized_original_path in processed_dll_paths
                ):
                    if merge:
                        results["mods_skipped"] += 1
                        results["skipped_details"].append(
                            f"DLL '{Path(original_path_str).name}' already exists in the active profile."
                        )
                    continue
                ...
                if full_path.exists() and full_path.suffix.lower() == ".dll":
                    ...
                    try:
                        ...
                        dll_paths_to_add.append(
                            (original_path_str, relative_to_import, True)
                        )  # True = internal to package
                    ...
                    if merge:
                        results["mods_skipped"] += 1
                        results["skipped_details"].append(
                            f"DLL '{Path(original_path_str).name}' not found at expected path."
                        )
                    else:
                        results["errors"].append(
                            f"DLL mod from profile not found at expected path: {full_path}"
                        )
            ...
            if custom_mod_name:
                ...
                if new_mod_folder.exists() and merge:
                    ...
                    results["mods_skipped"] += 1
                    results["skipped_details"].append(
                        f"Package '{final_mod_name}' already exists in the mods folder."
                    )
                else:
                    ...
                    folders_to_copy = []
                    source_dirs_map = {
                        p.name.lower(): p for p in import_path.iterdir() if p.is_dir()
                    }
                    ...
                    if folders_to_copy:
                        try:
                            ...
                            for src_folder in folders_to_copy:
                                shutil.copytree(
                                    src_folder, new_mod_folder, dirs_exist_ok=True
                                )
                            ...
                            if final_mod_name not in {
                                p.get("id") for p in current_config.get("packages", [])
                            }:
                            ...
                            results["errors"].append(
                                f"Failed to create bundled mod '{final_mod_name}': {str(e)}"
                            )
                            if new_mod_folder.exists():
                                shutil.rmtree(new_mod_folder)
            ...
            for original_path, relative_path, is_internal in dll_paths_to_add:
                if is_internal and custom_mod_name:
                    ...
                    mods_dir_name = self.games[game_name]["mods_dir"]
                    new_dll_path = (
                        f"{mods_dir_name}\\{final_mod_name}\\{relative_path}".replace(
                            "/", "\\"
                        )
                    )
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                try:
                    ...
                    result = subprocess.run(
                        ["pgrep", "steam"], capture_output=True, text=True, timeout=5
                    )
                    if result.returncode == 0:
                        return True
        ...
        try:
            ...
            if game_name in self.me3_config_paths:
                ...
                try:
                    ...
                    print(
                        f"Custom config path {custom_path} is no longer accessible: {e}"
                    )
                    del self.me3_config_paths[game_name]
                    self._save_settings()
            ...
            if hasattr(self, "me3_info") and self.me3_info:
                ...
                if (
                    existing_readable
                    and not self._is_writable(existing_readable)
                    and writable_location
                ):
                    # Copy the system config to user location if it doesn't exist there
                    if not writable_location.exists():
                        self._copy_config_to_writable_location(
                            existing_readable, writable_location
                        )
                ...
                return str(
                    existing_readable
                    if existing_readable and self._is_writable(existing_readable)
                    else writable_location
                )
    ...
    def _copy_config_to_writable_location(
        self, source_config: Path, target_config: Path
    ):
    ...
    def set_me3_config_path(self, game_name: str, config_path: str):
        ...
        try:
            ...
            if not (
                self._is_writable(config_path_obj)
                or self._is_parent_writable(config_path_obj)
            ):
            ...
            if not config_path_obj.exists():
                if hasattr(self, "me3_info") and self.me3_info:
                    ...
                    if not success:
                        raise Exception(
                            "Failed to create config file at selected location"
                        )
            ...
            for config_path in available_paths:
                if config_path != keep_config_path and config_path.exists():
                    ...
                    if self._is_system_path(config_path):
                        print(
                            f"Skipping cleanup of system config: {config_path} (requires root privileges)"
                        )
                        continue
                    ...
                    if self._is_writable(config_path):
                        try:
                            ...
                            print(
                                f"Could not remove duplicate config {config_path}: {e}"
                            )
                    else:
