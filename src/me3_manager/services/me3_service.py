...
class Me3Service:
    ...
    def fetch_all_releases(self, per_page: int = 30) -> list[dict]:
        ...
        try:
            releases = []
            if latest := self.fetch_latest_release():
            ...
            for r in resp.json():
                if not any(x.get("id") == r.get("id") for x in releases):
                    releases.append(r)
            ...
            for r in releases:
                if r.get("draft", False):
                    continue
                if self._parse_version(r.get("tag_name", "")) >= min_ver:
                    result.append(r)

            result.sort(
                key=lambda r: self._parse_version(r.get("tag_name", "")), reverse=True
            )
            return result
